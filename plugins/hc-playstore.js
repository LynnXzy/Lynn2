...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    if (!args[0]) return conn.reply(m.chat, `Masukkan Text!\n\ncontoh:\n${usedPrefix + command} Freefire`, m);

    conn.reply(m.chat, 'Sedang diproses, mohon tunggu...', m);
    m.react('⌛');
    const apiEndpoint = `https://rzky.my.id/api/search/playstore?query=${args[0]}&apikey=${rzky}`;
    ...
    if (Array.isArray(js.result) && js.result.length > 0) {
      ...
      for (const app of apps) {
        ...
        message += `❏ Nama : ${nama}\n❏ Developer : ${developer}\n❏ Link : ${link}\n❏ Image : ${img}\n❏ Rate : ${rate}\n❏ Rate 2 : ${rate2}\n❏ Developer Link : ${link_dev}\n\n`;
      }

      await conn.sendFile(m.chat, apps[0].img, null, message, m);
      m.react('✅');
    } else {
      m.react('❌');
