...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    if (!args[0]) return conn.reply(m.chat, `Masukkan Text Dengan Benar!\n\ncontoh:\n${usedPrefix + command} Demon`, m);

    conn.reply(m.chat, 'Sedang diproses, mohon tunggu...', m);
    m.react('⌛');
    const apiEndpoint = `https://rzky.my.id/api/anime/nekopoi/search?query=${args[0]}&apikey=${rzky}`;
