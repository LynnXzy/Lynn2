...
let handler = async (m, { conn, text, usedPrefix, command, args, participants, isOwner }) => {
   ...
   if (!args[0]) conn.reply(`✳️ Kirim Grup link\n\n 📌 Contoh:\n *${usedPrefix + command}* <linkwa> <Hari>\n\n_Jumlahnya adalah hari bot akan berada di grup_`)
   if (!code) conn.reply(`✳️ Tautan tidak valid`)
   if (!args[1]) conn.reply(`📌 Jumlah hari yang hilang\n\n Contoh:\n *${usedPrefix + command}* <linkwa> 2`)
   if (isNaN(args[1])) conn.reply(`✳️ Hanya angka, yang mewakili hari-hari bot akan berada di grup!`)
  ...
  if (e.length) await conn.reply(res, `🏮 Halo Rzky
 
 @${owbot} Dia adalah pencipta saya jika Anda memiliki pertanyaan saya diundang oleh *${m.name}*`, m, {
     mentions: d
      }).then(async () => {
      await delay(7000)
      }).then( async () => {
 ...
 *${conn.user.name}* adalah salah satu bot WhatsApp multi-perangkat yang dibuat dengan bahasa Node.js, *${conn.user.name}* Baru saja diundang oleh *${m.name}*
 
 Untuk melihat menu bot silahkan untuk mengetik  ${usedPrefix}menu
