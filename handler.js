        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    ...
                    if (name != 'owner-unbanuser.js' && user?.banned)
                        ...
                        if (m.text && user.banned && !isROwner) {
                            ...
                            m.reply(`ANDA DILARANG!\n
                            *ALASAN:* ${user.messageSpam === 0 ? 'TIDAK TERTENTU' : user.messageSpam}\n
                            \`\`\`JIKA ANDA BERPIKIR INI ADALAH BUG, ​​ANDA DAPAT MENGHUBUNGI PEMILIK ATAU MODERATOR SAYA`)
                            user.antispam++	
                            return
                            }}
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `Anda tidak memiliki diamond, beli lebih banyak diamond dengan \n${usedPrefix}buy kuantitas\n${usedPrefix}buy all`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `Anda memerlukan level ${plugin.level} untuk menggunakan perintah ini \nLevel Anda ${_user.level}`, m)
...
let wel = 'https://telegra.ph/file/f82218075a0986c430acf.jpg'
 ...
 let lea = 'https://telegra.ph/file/919591feb232790a9f4c9.jpg'
                ...
                break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user Sekarang dia adalah seorang administrator')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => './src/HC.png') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user Bukan lagi administrator')
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '_Maaf untuk akses bot anda perlu verify terlebih dahulu, silahkan ketik .verify nama_',
